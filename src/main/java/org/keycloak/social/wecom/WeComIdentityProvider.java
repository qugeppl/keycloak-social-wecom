...
public class WeComIdentityProvider extends AbstractOAuth2IdentityProvider<WeComProviderConfig>
    ...
    private String ACCESS_TOKEN_CACHE_KEY = "wecom_sso_access_token";

    private static DefaultCacheManager _cacheManager;
    public static String WECOM_WORK_CACHE_NAME = "wecom_sso";
    ...
    private static DefaultCacheManager getCacheManager() {
        if (_cacheManager == null) {
            ...
            _cacheManager.defineConfiguration(WECOM_WORK_CACHE_NAME, config.build());
        }
    ...
    private static Cache<String, String> get_cache() {
        try {
            Cache<String, String> cache = getCacheManager().getCache(WECOM_WORK_CACHE_NAME);
            logger.info(cache);
            return cache;
        } catch (Exception e) {
            logger.error(e);
    ...
    private String get_access_token(String realm) {
        try {
            String cacheKey = realm+"_"+ACCESS_TOKEN_CACHE_KEY;
            String token = sso_cache.get(cacheKey);
            if (token == null) {
                JsonNode j = _renew_access_token();
                String tokenRenew = object2String(j);
                if (j == null) {
                ...
                sso_cache.put(cacheKey, token, timeout, TimeUnit.SECONDS);
            }
            return token;
        } catch (Exception e) {
            logger.error(e);
...
//            logger.info("request wechat work access token " + j.toString());
    ...
    private String reset_access_token(String realm) {
        String cacheKey = realm+"_"+ACCESS_TOKEN_CACHE_KEY;
        sso_cache.remove(cacheKey);
        return get_access_token(realm);
    }
    ...
    public BrokeredIdentityContext getFederatedIdentity(String authorizationCode,String realm) {
        String accessToken = get_access_token(realm);
        if (accessToken == null) {
        ...
        try {
            ...
            if (profile.get("errcode") != null) {
                ...
                if (errcode == 42001 || errcode == 40014) {
                    accessToken = reset_access_token(realm);
                    profile = SimpleHttp.doGet(PROFILE_URL, session)
                            .param(ACCESS_TOKEN_KEY, accessToken)
                            .param("code", authorizationCode)
                            .asJson();
